...
import { createMobileUiController } from './ui/mobileUi.js';
...
let lifecycleSuspendedSim = false;
let lifecycleSuspendedPaused = false;
let lifecycleSuspendedSandboxPaused = false;
...
let defaultCreatureCatalogPromise = null;
...
function loadDefaultCreatureCatalogEntries() {
  if (!defaultCreatureCatalogPromise) {
    defaultCreatureCatalogPromise = import('./data/defaultCreatureCatalog.js')
      .then(({ buildDefaultCreatureCatalogEntries }) => buildDefaultCreatureCatalogEntries())
      .catch(error => {
        console.warn('Failed to load default creature catalog.', error);
        return [];
      });
  }
  return defaultCreatureCatalogPromise;
}
...
function pushTurboPerfInterval(key, now = performance.now()) {
  ...
  const lastKey = `last${key.charAt(0).toUpperCase()}${key.slice(1)}At`;
  const previous = turboPerfProbe[lastKey];
  if (previous > 0) {
    turboPerfProbe[listKey].push(now - previous);
    if (turboPerfProbe[listKey].length > 120) turboPerfProbe[listKey].shift();
  }
  turboPerfProbe[lastKey] = now;
}
...
const mobileUi = createMobileUiController({
  sim,
  storagePrefix: MOBILE_MODULE_STORAGE_PREFIX,
  isMobileRuntime: () => document.body.classList.contains('app-mobile'),
...
function setScreen(name) {
  ...
  if (name === 'draw') {
    ...
    resizeCanvases();
    worldCtx = getOptimized2dContext(worldCanvas, { opaque: true });
    simSessionStarted = false;
...
async function hydrateDefaultCreatureCatalog() {
  if (creatureCatalog.length) return;
  const defaultEntries = await loadDefaultCreatureCatalogEntries();
  if (creatureCatalog.length || !defaultEntries.length) return;
  creatureCatalog = buildCatalogRuntimeEntries(defaultEntries);
...
function toggleCreatureCatalog(forceOpen = null) {
  ...
  if (open) {
    ...
    renderCreatureCatalog();
    if (!creatureCatalog.length) void hydrateDefaultCreatureCatalog();
    if (!backdrop) {
...
function startTurboSmokeSession(options = {}) {
  const {
    popSize = 16,
    simDuration = 10,
    simSpeed = 1,
  ...
  sim.simDuration = Math.max(4, Number(simDuration) || 10);
...
function suspendSimulationForLifecycle() {
  if (!simSessionStarted || currentScreen !== 'sim') return;
  if (!sim.frameId && !sim._turboRunning) return;

  lifecycleSuspendedPaused = !!sim.paused;
  lifecycleSuspendedSandboxPaused = !!sim.sandboxPaused;
  lifecycleSuspendedSim = !(sim.sandboxMode ? sim.sandboxPaused : sim.paused);

  sim.paused = true;
  if (sim.sandboxMode) sim.sandboxPaused = true;
  sim.stopLoop();
  updateStartSimUI();
  syncSandboxPauseButtons();
}

function resumeSimulationFromLifecycle() {
  if (!lifecycleSuspendedSim || !simSessionStarted || currentScreen !== 'sim') return;
  if (document.visibilityState === 'hidden') return;

  sim.paused = lifecycleSuspendedPaused;
  if (sim.sandboxMode) sim.sandboxPaused = lifecycleSuspendedSandboxPaused;
  lifecycleSuspendedSim = false;

  if (!(sim.sandboxMode ? sim.sandboxPaused : sim.paused)) {
    if (sim.sandboxMode) {
      if (!sim.frameId) {
        sim.lastFrame = performance.now();
        sim.frameId = requestAnimationFrame(ts => sim.gameLoop(ts));
...
document.addEventListener('visibilitychange', () => {
  if (document.visibilityState === 'hidden') {
    suspendSimulationForLifecycle();
  } else if (document.visibilityState === 'visible') {
    resumeSimulationFromLifecycle();
  }
});

window.addEventListener('pagehide', () => {
  // Safari/iOS PWAs frequently transition through page cache; stop hot loops eagerly.
  suspendSimulationForLifecycle();
});
...
if (localCatalogRecord && localCatalogRecord.items.length) {
  ...
  creatureCatalog = [];
  creatureCatalogUpdatedAt = 0;
  void hydrateDefaultCreatureCatalog();
}
renderCreatureCatalog();
...
function drawTurboGenerationPoles(ctx, groundY) {
  ...
  const compactMobile = document.body.classList.contains('app-mobile');
  const total = history.length;
  const startIndex = compactMobile ? Math.max(0, total - 8) : 0;
  ctx.save();
  for (let i = startIndex; i < total; i++) {
    ...
    if (compactMobile) {
      ctx.strokeStyle = `rgba(34, 211, 238, ${Math.max(0.18, alpha * 0.9)})`;
...
({ leftW, rightW, topH, bottomH } = cachedPanelDims);
  ...
  const markerSpacingM = (document.body.classList.contains('app-mobile') && sim.trainingMode === 'turbo') ? 20 : 10;
...
function isMobileStatsSheetVisible() {
  return document.body.classList.contains('app-mobile')
    && document.body.classList.contains('mobile-sheet-open')
    && document.body.classList.contains('mobile-sheet-stats');
}

function isMobileControlsSheetVisible() {
  return document.body.classList.contains('app-mobile')
    && document.body.classList.contains('mobile-sheet-open')
    && document.body.classList.contains('mobile-sheet-controls');
}
...
if (E2E_ENABLED) {
  window.__creatureLabsE2E = {
    loadDesign(payload) {
      designer.loadDesign(payload);
      return designer.getDesign();
    },
    goToScreen(name) {
      setScreen(name === 'sim' ? 'sim' : (name === 'splash' ? 'splash' : 'draw'));
      return currentScreen;
    },
    startTurboSmokeSession(options = {}) {
      return startTurboSmokeSession(options);
    },
    resetTurboPerfProbe() {
      resetTurboPerfProbe();
    },
